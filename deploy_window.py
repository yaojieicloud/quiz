...
client.connect(ECS_HOST, 22, username=ECS_USER, password=ECS_PASS, timeout=15, allow_agent=False, look_for_keys=False)
...
# 2) 解压（更新 src 目录）
cmd = "cd /opt/quiz-system && tar -xzf quiz-src-window.tar.gz && ls src/core/mastery.py"
...
# 3) 重建镜像
cmd = "cd /opt/quiz-system && docker build -t quiz-system:latest -f src/Dockerfile src/ 2>&1 | tail -10"
