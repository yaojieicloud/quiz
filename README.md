...
| [管理后台与学情分析](docs/design/管理后台与学情分析.md) | 10 tab 总台、学情分析 5 子页、最近动态接口、记录管理、掌握度矩阵 |
...
- 积分发放档位默认 100→5 / 90→4 / 80→3，<80 分不计分；与成绩同事务落 `points_ledger`。Python 实操科目覆盖为 100→2 / 90→1 / <90→0（`subject_points`，防刷分）；**课程达「精通」则该次不再发分**（掌握度闸门）。
